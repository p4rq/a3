...
router.get('/exchangeRate', async (req, res) => {
  try {
    const exchangeRateData = await ExchangeRate.find().sort({ timestamp: 'desc' });

    // Render EJS template for exchange rate data
    res.render('download/exchangeRate', { exchangeRateData }, (err, html) => {
      if (err) {
        console.error(err);
        return res.status(500).send('Internal Server Error');
      }
      ...
      pdf.create(html).toBuffer((err, buffer) => {
        ...
        // Send PDF as response
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', 'attachment; filename=exchangeRateData.pdf');
        res.send(buffer);
      });
    });
  } catch (err) {
